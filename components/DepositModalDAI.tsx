...
import { PDAI_OPTIMISM, daiToken } from "@/lib/constants";
import Image from "next/image";

export default function DepositModalDAI({ connectedAddress, publicClient }: any) {
      ...
      {showContinue ? (
        ...
        <button
          ...
          onClick={() => confirmRoute({
            ...
            dstInputVal: dstDisplay!,
            contractAddress: `0x${PDAI_OPTIMISM?.slice(2)}`,
            signature: DEPOSIT_SIGNATURE,
            args: [parseUnits(dstDisplay!, 18), connectedAddress],
            connectedAddress,
            continueDisabled,
            setSubmitting,
            setShowContinue,
        ...
        <button
          className={
            `${confirmDisabled ? 'bg-gray-300 text-gray-600 ': 'bg-purple-500 text-white '}` +
            "text-center font-medium" +
            " w-full rounded-lg p-2 mt-4" +
            " relative flex items-center justify-center"
          }
