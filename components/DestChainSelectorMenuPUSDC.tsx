...
import { destChainIconsPUSDC, destChainNamesPUSDC } from "../lib/contexts/routeSelectContextPUSDC";
    ...
    <Popover className={className}>
      {({ close }) => (
        <>
          <Popover.Button>
            {renderBtnInner ? (
              renderBtnInner(destChainNamesPUSDC[chainId], destChainIconsPUSDC[chainId])
            ) : (
              <div className="border border-[#BEC3C9] rounded px-2 leading-none inline-flex items-baseline py-1.5">
                <span className="self-center">
                  <Image
                    ...
                    width="14"
                    height="14"
                    alt={destChainNamesPUSDC[chainId]}
                    src={destChainIconsPUSDC[chainId]}
                  />
                </span>
                <span className="ml-1">{destChainNamesPUSDC[chainId]}</span>
              </div>
            )}
          </Popover.Button>
          <Transition
                ...
                {availableChains.map((c) => (
                  ...
                  >
                    <Image
                      ...
                      width="14"
                      height="14"
                      src={destChainIconsPUSDC[c]}
                      alt={destChainNamesPUSDC[c]}
                    />
                    <div className="ml-1">{destChainNamesPUSDC[c]}</div>
                  </button>
                ))}
              </div>
            </Popover.Panel>
