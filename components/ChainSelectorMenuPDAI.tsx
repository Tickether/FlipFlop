...
import { chainIconsPDAI, chainNamesPDAI } from "../lib/contexts/routeSelectContextPDAI";
...
export default function ChainSelectMenuPDAI({
  chainId,
  onSelectChain,
    ...
    <Popover className={className}>
      {({ close }) => (
        <>
          <Popover.Button>
            {renderBtnInner ? (
              renderBtnInner(chainNamesPDAI[chainId], chainIconsPDAI[chainId])
            ) : (
              <div className="border border-[#BEC3C9] rounded px-2 leading-none inline-flex items-baseline py-1.5">
                <span className="self-center">
                  <Image
                    ...
                    width="14"
                    height="14"
                    alt={chainNamesPDAI[chainId]}
                    src={chainIconsPDAI[chainId]}
                  />
                </span>
                <span className="ml-1">{chainNamesPDAI[chainId]}</span>
              </div>
            )}
          </Popover.Button>
          <Transition
                ...
                {availableChains.map((c) => (
                  ...
                  >
                    <Image
                      ...
                      width="14"
                      height="14"
                      src={chainIconsPDAI[c]}
                      alt={chainNamesPDAI[c]}
                    />
                    <div className="ml-1">{chainNamesPDAI[c]}</div>
                  </button>
                ))}
              </div>
            </Popover.Panel>
