...
import { chainIconsPUSDC, chainNamesPUSDC } from "../lib/contexts/routeSelectContextPUSDC";
...
export default function ChainSelectMenuPUSDC({
  chainId,
  onSelectChain,
    ...
    <Popover className={className}>
      {({ close }) => (
        <>
          <Popover.Button>
            {renderBtnInner ? (
              renderBtnInner(chainNamesPUSDC[chainId], chainIconsPUSDC[chainId])
            ) : (
              <div className="border border-[#BEC3C9] rounded px-2 leading-none inline-flex items-baseline py-1.5">
                <span className="self-center">
                  <Image
                    ...
                    width="14"
                    height="14"
                    alt={chainNamesPUSDC[chainId]}
                    src={chainIconsPUSDC[chainId]}
                  />
                </span>
                <span className="ml-1">{chainNamesPUSDC[chainId]}</span>
              </div>
            )}
          </Popover.Button>
          <Transition
                ...
                {availableChains.map((c) => (
                  ...
                  >
                    <Image
                      ...
                      width="14"
                      height="14"
                      src={chainIconsPUSDC[c]}
                      alt={chainNamesPUSDC[c]}
                    />
                    <div className="ml-1">{chainNamesPUSDC[c]}</div>
                  </button>
                ))}
              </div>
            </Popover.Panel>
