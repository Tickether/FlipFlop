...
import { destChainIconsPDAI, destChainNamesPDAI } from "../lib/contexts/routeSelectContextPDAI";
    ...
    <Popover className={className}>
      {({ close }) => (
        <>
          <Popover.Button>
            {renderBtnInner ? (
              renderBtnInner(destChainNamesPDAI[chainId], destChainIconsPDAI[chainId])
            ) : (
              <div className="border border-[#BEC3C9] rounded px-2 leading-none inline-flex items-baseline py-1.5">
                <span className="self-center">
                  <Image
                    ...
                    width="14"
                    height="14"
                    alt={destChainNamesPDAI[chainId]}
                    src={destChainIconsPDAI[chainId]}
                  />
                </span>
                <span className="ml-1">{destChainNamesPDAI[chainId]}</span>
              </div>
            )}
          </Popover.Button>
          <Transition
                ...
                {availableChains.map((c) => (
                  ...
                  >
                    <Image
                      ...
                      width="14"
                      height="14"
                      src={destChainIconsPDAI[c]}
                      alt={destChainNamesPDAI[c]}
                    />
                    <div className="ml-1">{destChainNamesPDAI[c]}</div>
                  </button>
                ))}
              </div>
            </Popover.Panel>
